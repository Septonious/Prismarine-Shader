...
void NormalFog(inout vec3 color, vec3 viewPos, bool isDense) {
	#if DISTANT_FADE > 0
	...
	#ifdef OVERWORLD
	float fog = length(viewPos) * (FOG_DENSITY * (1.0 + float(isDense) + float(isDense) + float(isDense))) / 256.0;
